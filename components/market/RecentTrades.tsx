...
export default function RecentTrades() {
  ...
  const loadTrades = useCallback(async (silent = false) => {
    ...
    if (loadError) {
      console.error("Failed to load recent trades:", loadError);

      setError("Recent trades are temporarily unavailable.");
  ...
  function getSideStyles(side: TradeSide) {
    switch (side) {
      case "BUY":
        return {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="divide-y divide-white/[0.05]">
              {trades.map((trade) => {
                const side = getTradeSide(
                  trade.execution_type
                );

                const styles = getSideStyles(side);
